...
namespace Perlink.Oi.Juridico.WebApi.Areas.Manutencao
{
    ...
    public class EscritorioController : ApiControllerBase
    {
        ...
        {
            return Result(repository.ObterPaginado(estado,areaAtuacao, pagina, quantidade, EnumHelpers.ParseOrDefault(coluna, EscritorioSort.Id), string.IsNullOrEmpty(direcao) || direcao.Equals("asc"), pesquisa));
        }
        ...
        public IActionResult Criar(
            [FromServices] IEscritorioService service,
            [FromBody] CriarEscritorioCommand dados)
        {
            return Result(service.Criar(dados));
        }

        ///<summary>
        /// Atualiza registro de Escritorio
        ///</summary>
        ...
        public IActionResult Atualizar(
            [FromServices] IEscritorioService service,
            [FromBody] AtualizarEscritorioCommand dados)
        {
            ...
            [FromQuery] string pesquisa = null)
        {
            ...
            if (resultado.Tipo == ResultType.Valid)
            {
                ...
                foreach (var a in resultado.Dados)
                {
                    ...
                    csv.Append($"\"{ (a.Enviar_App_Preposto == true ? "Sim" : "Não") }\";");
             ...
             [FromQuery] string pesquisa = null)
        {
            ...
            var advogados = repositoryAdvogado.ObterTodos(AdvogadodoEscritorioSort.Nome, true).Dados;
        
            if (resultado.Tipo == ResultType.Valid)
            {
                StringBuilder csv = new StringBuilder();

                csv.AppendLine($"Nome;CPF;CNPJ;Estado OAB;Nro OAB;Nome Advogado;DDD;Celular;Contato do Escritório");

                foreach (var a in resultado.Dados)
                {
                    var advogado = advogados.Where(x => x.EscritorioId == a.Id);

                    foreach (var b in advogado)
                    {
                        var cnpj = a.CNPJ != null ? $"'{a.CNPJ}" : string.Empty;
                        var cpf = a.CPF != null ? $"'{a.CPF}" : string.Empty;
                        ...
                        csv.Append($"\"{ cnpj  }\";");
                        csv.Append($"\"{ cpf  }\";");
                        csv.Append($"\"{(b.EstadoId)}\";");
                        csv.Append($"\"{(b.NumeroOAB)}\";");                        
                        csv.Append($"\"{(b.Nome)}\";");
                        csv.Append($"\"{(b.CelularDDD)}\";");
                        csv.Append($"\"{(b.Celular)}\";");
                        csv.Append($"\"{(b.EhContato == true ? "Sim" : "Não")}\";");
                        csv.AppendLine("");
                    }           
