...
{
    public class CriarParteCommand : Validatable, IValidatable
    {
        ...
        public decimal? ValorCartaFianca { get; set; } = 0;
        ...
        public override void Validate()
        {
            ...
            if (!string.IsNullOrEmpty(EstadoId)) {
                var estado = EnumEstado.PorId(EstadoId);

                if (estado == null || (estado != null && estado.Nome == estado.Id)) {
                    AddNotification(nameof(EstadoId), "O Estado informado não existe");
                }
            }
        }
    }
