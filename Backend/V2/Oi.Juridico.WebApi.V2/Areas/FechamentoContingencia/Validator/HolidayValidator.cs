namespace Oi.Juridico.WebApi.V2.Areas.FechamentoContingencia.Validator
{
    ...
    {
        private static IList<DateTime> GetHolidaysByCurrentYear(int? yearParameter = null)
        {
        ...
        {
            var listHolidays = GetHolidaysByCurrentYear(data.Year);

            foreach(var holiday in listHolidays)
            {
                if (holiday.Equals(data)) return true;
            }
