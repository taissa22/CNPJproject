...
{
    public class ExportaAcompanhamentoEnvioMap : ClassMap<AcompanhamentoEnvioDTO>
    {
        public ExportaAcompanhamentoEnvioMap()
        {
            ...
            Map(row => row.Codigo).Index(11).Convert(x => $"\t{x.Value.Codigo}").Name("Código");
            Map(row => row.Ocorrencia).Index(12).Name("Ocorrência");
            Map(row => row.Acao).Index(13).Name("Ação");
            Map(row => row.Localizacao).Index(14).Convert(x => $"\t{x.Value.Localizacao}").Name("Localização");
            Map(row => row.IdExecucao).Index(15).Name("Id Execução");
