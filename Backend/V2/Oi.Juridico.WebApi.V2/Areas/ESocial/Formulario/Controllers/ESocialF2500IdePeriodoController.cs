...
namespace Oi.Juridico.WebApi.V2.Areas.ESocial.Formulario.Controllers
{
    ...
    public class ESocialF2500IdePeriodoController : ControllerBase
    {
        ...
        [HttpGet("consulta/verifica/ide-periodo/{codigoContrato}")]
        [TemPermissao(Permissoes.ACESSAR_CADASTRO_ESOCIAL)]
        public async Task<ActionResult<bool>> VerificaPeriodosF2500dAsync([FromRoute] int codigoContrato, CancellationToken ct)
        {
            try
            {
                var contrato = await _eSocialDbContext.EsF2500Infocontrato.AsNoTracking().FirstOrDefaultAsync(x => x.IdEsF2500Infocontrato == codigoContrato);
                if (contrato is not null)
                {
                    var periodo = await _eSocialDbContext.EsF2500Ideperiodo.AsNoTracking().AnyAsync(x => x.IdEsF2500Infocontrato == codigoContrato, ct);
                    return Ok(periodo);
                }
            ...
            try
            {
                ...
                if (formulario is not null)
                {
                    if (contrato is not null)
                    {
                        ...
                        {
                                var infoInterm = _eSocialDbContext.EsF2500Infointerm.Any(x => x.IdEsF2500Ideperiodo == periodo.IdEsF2500Ideperiodo);
                                if (infoInterm)
                                {
                                    return BadRequest($"Quando o  campo \"Possui Inf. Evento Admissão/Início (Bloco D)\" for diferente de \"Não\" e a Categoria informada (Bloco D) for diferente de \"111 – EMPREGADO – CONTRATO DE TRABALHO INTERMITENTE\" o grupo \"Informações referentes ao Trabalho Intermitente (Bloco K)\" não deve ser preenchido para o periodo.");
                                }
                        ...
                        listaErros.AddRange(service.ValidaAlteracaoIdePeriodo(requestDTO, contrato, _eSocialDbContext, periodo.IdEsF2500Ideperiodo).ToList());
        ...
        [TemPermissao(Permissoes.ACESSAR_CADASTRO_ESOCIAL)]
        public async Task<ActionResult> CadastraIdePeriodoF2500Async([FromRoute] int codigoFormulario, [FromRoute] int codigoContrato, [FromBody] EsF2500IdeperiodoRequestDTO requestDTO, [FromServices] ESocialF2500IdePeriodoService service, [FromServices] ESocialF2500InfoIntermService serviceInfoInterm, CancellationToken ct)
        {
            try
            {
                if (!_controleDeAcessoService.TemPermissao(Permissoes.ESOCIAL_ALTERAR_BLOCOGK))
                ...
                if (formulario is not null)
                {
                    if (contrato is not null)
                    {
                        ...
                        if (await _eSocialDbContext.EsF2500Ideperiodo.Where(x => x.IdEsF2500Infocontrato == codigoContrato).CountAsync(ct) >= 999)
                        {
                            return BadRequest("O sistema só permite a inclusão de até 999 Bases de Cálculos.");
                        };
                        ...
                        listaErros.AddRange(service.ValidaInclusaoIdePeriodo(requestDTO, contrato, _eSocialDbContext).ToList());
                        ...
                        if (contrato.InfocontrIndcontr == "N"
                            && contrato.InfocontrCodcateg == 111)
                        {
                            if (!await serviceInfoInterm.ExisteInfoIntermPorPeriodoAsync(periodo.IdEsF2500Ideperiodo,ct)) {
                                serviceInfoInterm.InsereInfoIntermZerado(User, ct, periodo.IdEsF2500Ideperiodo);
                            }
                        }
            ...
            try
            {
                ...
                using (var reader = new StreamReader(arquivo.OpenReadStream()))
                {
                    ...
                    using (var csv = new CsvReader(reader, csvConfiguration))
                    {
                        ...
                        if (linhas.Count > 1000)
                        {
                            return BadRequest("Quantidade de linhas no arquivo excedida. Quantidade máxima 999 linhas.");
                        }
                        ...
                        if (formulario is not null)
                        {
                            if (contrato is not null)
                            {
                                ...
                                foreach (var linha in linhas)
                                {
                                    ...
                                    listaErros.AddRange(service.ValidaInclusaoIdePeriodo(linha, contrato, _eSocialDbContext).ToList());
            ...
            try
            {
                ...
                if (formulario is not null)
                {
                    if (contrato is not null)
                    {
                        ...
                        var periodo = await _eSocialDbContext.EsF2500Ideperiodo.FirstOrDefaultAsync(x => x.IdEsF2500Infocontrato == codigoContrato && x.IdEsF2500Ideperiodo == codigoIdeperiodo, ct);
                        var infoInterm = await _eSocialDbContext.EsF2500Infointerm.Where(x => x.IdEsF2500Ideperiodo == codigoIdeperiodo).ToListAsync(ct);
        ...
        private void PreencheIdePeriodo(ref EsF2500Ideperiodo periodo, EsF2500IdeperiodoRequestDTO requestDTO, int? codigoContrato = null)
        {
            ...
            periodo.InfofgtsVrbcfgtsproctrab = requestDTO.InfofgtsVrbcfgtsproctrab;
            periodo.InfofgtsVrbcfgtssefip = requestDTO.InfofgtsVrbcfgtssefip;
            periodo.InfofgtsVrbcfgtsdecant = requestDTO.InfofgtsVrbcfgtsdecant;
        ...
        private static EsF2500IdeperiodoDTO PreencheIdePeriodoDTO(ref EsF2500Ideperiodo? periodo)
        {
            return new EsF2500IdeperiodoDTO()
            {
                ...
                InfofgtsVrbcfgtsproctrab = periodo!.InfofgtsVrbcfgtsproctrab,
                InfofgtsVrbcfgtssefip = periodo!.InfofgtsVrbcfgtssefip,
                InfofgtsVrbcfgtsdecant = periodo!.InfofgtsVrbcfgtsdecant,
        ...
        private IQueryable<EsF2500IdeperiodoDTO> RecuperaListaPeriodos(int codigoContrato)
        {
            return from esf2500 in _eSocialDbContext.EsF2500Ideperiodo.AsNoTracking()
                   ...
                   select new EsF2500IdeperiodoDTO()
                   {
                       ...
                       InfofgtsVrbcfgtsproctrab = esf2500.InfofgtsVrbcfgtsproctrab,
                       InfofgtsVrbcfgtssefip = esf2500.InfofgtsVrbcfgtssefip,
                       InfofgtsVrbcfgtsdecant = esf2500.InfofgtsVrbcfgtsdecant,
                   };
        }
