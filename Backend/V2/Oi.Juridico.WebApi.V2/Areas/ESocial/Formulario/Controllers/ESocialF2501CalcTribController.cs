        ...
        public async Task<ActionResult> AlteraCalcTribF2501Async([FromRoute] int codigoFormulario, [FromRoute] int codigoCalctrib, [FromBody] EsF2501CalctribRequestDTO requestDTO, CancellationToken ct)
        {
            try
            {
                ...
                if (formulario is not null)
                {
                    ...
                    var dataMinimaPeriodoReferencia = new DateTime(2008, 12, 01);

                    if (requestDTO.CalctribPerref < dataMinimaPeriodoReferencia)
                    {
                        listaErros.Add($"O período de referência informado deve ser maior ou igual a 12/2008.");
                    }
        ...
        public async Task<ActionResult> CadastraCalcribF2501Async([FromRoute] int codigoFormulario, [FromBody] EsF2501CalctribRequestDTO requestDTO, CancellationToken ct)
        {
            try
            {
                ...
                if (formulario is not null)
                {
                    ...
                    if (await _eSocialDbContext.EsF2501Calctrib.Where(x => x.IdEsF2501 == codigoFormulario).CountAsync(ct) >= 999)
                    {
                    ...
                    using (var csv = new CsvReader(reader, configuration))
                    {
                        ...
                        if (formulario is not null)
                        {
                            ...
                            foreach (var linha in linhas)
                            {
                                ...
                                if (linha.CalctribPerref < dataMinimaPeriodoReferencia)
                                {
                                    listaErros.Add($"O período de referência informado deve ser maior ou igual a 12/2008.");
                                }
                            ...
                            if (await _eSocialDbContext.EsF2501Calctrib.Where(x => x.IdEsF2501 == codF2501).CountAsync(ct) > 999)
                            {
                                scope.Rollback();
                                return BadRequest("O sistema só permite a inclusão de até 999 Bases de Cálculos.");
                            };
