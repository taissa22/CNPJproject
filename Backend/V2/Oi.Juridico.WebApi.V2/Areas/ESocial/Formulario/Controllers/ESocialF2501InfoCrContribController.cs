...

        {
            try
            {
                ...
                if (formulario is not null)
                {
                    ...
                    if (await _eSocialDbContext.EsF2501Infocrcontrib.AnyAsync(x => x.IdEsF2501Calctrib == codigoCalcTrib && x.IdEsF2501Infocrcontrib != codigoInfocrcontrib && x.InfocrcontribTpcr == requestDTO.InfocrcontribTpcr, ct))
                    {
                        listaErros.Add("O formulário pode conter apenas um registro para cada código de receita de informações de contribuições.");
                    }
        ...
        public async Task<ActionResult> CadastraInfocrcontribF2501Async([FromRoute] int codigoFormulario, [FromRoute] int codigoCalctrib, [FromBody] EsF2501InfocrcontribRequestDTO requestDTO, CancellationToken ct)
        {
            try
            {
                ...
                if (formulario is not null)
                {
                    ...
                    if (await _eSocialDbContext.EsF2501Infocrcontrib.AnyAsync(x => x.IdEsF2501Calctrib == codigoCalctrib && x.InfocrcontribTpcr == requestDTO.InfocrcontribTpcr, ct))
                    {
                        listaErros.Add("O formulário pode conter apenas um registro para cada código de receita de informações de contribuições.");
                    }
