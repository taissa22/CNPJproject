...
namespace Oi.Juridico.WebApi.V2.Areas.ESocial.Formulario.Controllers
{
    ...
    public class ESocialF2501Controller : ControllerBase
    {
            ...
            try
            {
                ...
                var formulario2501 = await (from es2501 in _eSocialDbContext.EsF2501.AsNoTracking()
                                            ...
                                            select new EsF2501HeaderDTO()
                                            {
                                                ...
                                                OkSemRecibo = es2501.OkSemRecibo,
        ...
        [TemPermissao(Permissoes.ACESSAR_CADASTRO_ESOCIAL)]
        public async Task<ActionResult> AlteraFormulario2501Async([FromRoute] int codigoFormulario, [FromRoute] byte statusFormulario, [FromBody] EsF2501RequestDTO requestDTO, [FromServices] ESocialF2501Service service, CancellationToken ct)
        {
            try
            {
                ...
                if (formulario2501 is not null)
                {
                    ...
                    if (formulario2501.InfoircomplemDtlaudo != requestDTO.InfoircomplemDtlaudo && !_controleDeAcessoService.TemPermissao(Permissoes.ENVIAR_2501_PARA_ESOCIAL))
                    {
                        return BadRequest("Você não possúi permissão para alterar o campo \"Data Moléstia Grave (Laudo)\".");
                    }
                    ...
                    if (statusFormulario == EsocialStatusFormulario.ProntoParaEnvio.ToByte())
                    {
                        ...
                        var listaErrosFilhos = await service.FilhosInvalido(_eSocialDbContext, codigoFormulario, ct);
        ...
        [HttpPatch("cadastro/alteracao/f2501/retorna-rascunho/{codigoFormulario}")]
        [TemPermissao(Permissoes.ESOCIAL_RETORNA_STATUS_RASCUNHO)]
        public async Task<ActionResult> RetornaFormulario2501StatusRascunhoAsync([FromRoute] int codigoFormulario, CancellationToken ct)
        {
            try
            {
                if (!_controleDeAcessoService.TemPermissao(Permissoes.CADASTRAR_FORM2501))
        ...
        [HttpPatch("cadastro/alteracao/f2501/atualiza-recibo/{codigoFormulario}")]
        [TemPermissao(Permissoes.ESOCIAL_ATUALIZAR_RECIBO)]
        public async Task<ActionResult> AlterarNumeroReciboAsync([FromRoute] int codigoFormulario, [FromBody] string? numeroRecibo, [FromServices] ESocialF2501Service service, CancellationToken ct)
        {
            try
            {
        ...
        [TemPermissao(Permissoes.ACESSAR_CADASTRO_ESOCIAL)]
        public async Task<ActionResult> ValidaFinalizacaoContadorF2501Async([FromRoute] int codigoFormulario, [FromServices] ESocialF2501Service service, CancellationToken ct)
        {
            try
            {
                var formulario2501 = await _eSocialDbContext.EsF2501.FirstOrDefaultAsync(x => x.IdF2501 == codigoFormulario, ct);
        ...
        #region Validação Numero Processo e Periodo

        [HttpGet("consulta/f2501/validar-processo-periodo/{nroProcesso}/{periodoApuracao}")]
        [TemPermissao(Permissoes.ACESSAR_CADASTRO_ESOCIAL)]
        public async Task<ActionResult> ValidaFormulario2501ProcessoPeriodoAsync([FromRoute] string nroProcesso, [FromRoute] string periodoApuracao, CancellationToken ct)
        {
            try
            {
                return Ok((await _eSocialDbContext.EsF2501.AsNoTracking().Where(x => x.IdeprocNrproctrab == nroProcesso && x.IdeprocPerapurpgto == periodoApuracao).ToListAsync(ct)).Count >= 1);
            }
            catch (Exception e)
            {
                return BadRequest(new { mensagem = "Falha ao recuperar dados do formulário.", erro = e.Message });
        ...
        private void PreencheEntidadeFormulario2501(EsF2501RequestDTO requestDTO, ref EsF2501? formulario2501)
        {
            ...
            formulario2501!.InfoircomplemDtlaudo = requestDTO.InfoircomplemDtlaudo.HasValue ? requestDTO.InfoircomplemDtlaudo.Value.Date : requestDTO.InfoircomplemDtlaudo;
        }
        ...
        private EsF2501DTO PreencheFormulario2501DTO(ref EsF2501? formulario2501)
        {
            var formularioDTO = new EsF2501DTO()
            {
                ...
                InfoircomplemDtlaudo = formulario2501!.InfoircomplemDtlaudo,
                OkSemRecibo = formulario2501!.OkSemRecibo,
            };
            ...
            if (formulario2501Exclusao.Count > 0)
            {
                foreach (var formulario2501 in formulario2501Exclusao)
                {
                    if (formulario2501 is not null)
                    {
                        ...
                        var listaFormulario2501InfodepExclusao = await _eSocialDbContext.EsF2501Infodep.Where(x => x.IdEsF2501 == formulario2501.IdF2501).ToListAsync(ct);
