...
{
    public class ListarPautaProconComposicaoAudiencia
    {
        ...
        public ListarPautaProconComposicaoAudiencia()
        {

        }

        public string PreparaListaAudienciaParaSql(ListarPautaProconComposicaoAudiencia model, string usuario)
        {
            var sql = String.Format(@"SELECT TO_CHAR(AP.HOR_AUDIENCIA,'hh24:mi') as HOR_AUDIENCIA, TAU.SGL_TIPO_AUDIENCIA, P.NRO_PROCESSO_CARTORIO || '<br>' || PA.NOM_PARTE AS NRO_PROCESSO_CARTORIO_EMPRESA,
                                      ...
                                      WHERE ap.COD_PROCESSO = p.COD_PROCESSO  
                                        ...
                                        AND exists (SELECT 1
                                                    ...
                                                    AND ur.cod_usuario = '{5}') ",
                                                    (short)TipoProcessoEnum.Procon,
                                                    model.DataAudiencia,
                                                    model.CodComarca,
                                                    model.CodVara,
                                                    model.CodTipoVara,
        ...
        public List<ListarPautaProconComposicaoAudienciaResponse> DadosListaAudiencia(DbDataReader reader, ListarPautaProconComposicaoAudiencia model)
        {
            List<ListarPautaProconComposicaoAudienciaResponse> lstAudiencia = new List<ListarPautaProconComposicaoAudienciaResponse>();

            while (reader.Read())
            {
                ListarPautaProconComposicaoAudienciaResponse audiencia = new ListarPautaProconComposicaoAudienciaResponse();
