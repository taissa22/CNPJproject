...
{
    public class ListarPautaCivelConsumidorComposicao
    {
        ...
        public string RequerPreposto { get; set; }

        public int AlocacaoTipo { get; set; } 

        public ListarPautaCivelConsumidorComposicao()
        {

        }

        public string PreparaListaPautaParaSql(ListarPautaCivelConsumidorComposicao model, string usuario)
        {
            ...
            if (model.RequerPreposto == "S" || model.RequerPreposto == "N")
                sql += " AND ap.IND_REQUER_PREPOSTO = '" + model.RequerPreposto + "' ";
        ...
        public string PreparaListaPautaSomenteJuizadoParaSql(ListarPautaCivelConsumidorComposicao model, string usuario)
        {
            var sql = String.Format(@"SELECT TO_CHAR(ap.DAT_AUDIENCIA, 'dd/mm/yyyy') as DAT_AUDIENCIA, p.COD_COMARCA, p.COD_TIPO_VARA, p.COD_VARA, c.COD_ESTADO, C.NOM_COMARCA, tv.NOM_TIPO_VARA
                         ...
                         WHERE p.COD_COMARCA = c.COD_COMARCA
                            ...
                            AND AP.DAT_AUDIENCIA <= '{1}'", model.PeriodoInicio, model.PeriodoFim, usuario, (short)TipoProcessoEnum.CivelConsumidor);
        ...
        public List<ListarPautaCivelConsumidorComposicaoResponse> DadosListaPauta(DbDataReader reader, ListarPautaCivelConsumidorComposicao model)
        {
            List<ListarPautaCivelConsumidorComposicaoResponse> lstPauta = new List<ListarPautaCivelConsumidorComposicaoResponse>();

            while (reader.Read())
            {
                ListarPautaCivelConsumidorComposicaoResponse pauta = new ListarPautaCivelConsumidorComposicaoResponse();
                ...
                pauta.Juizado = pauta.CodEstado + " - " + reader["NOM_COMARCA"].ToString() + " - " + pauta.CodVara + "º VARA " + reader["NOM_TIPO_VARA"].ToString();
