...
export class RelatorioPagamentoEscritorioService {

  constructor(private http: HttpClient, private exportarService: TransferenciaArquivos) {}

  private readonly endpoint = environment.api_v2_url + '/api/RelatorioPagamentoEscritorio';

  async obterAgendamentos(
    page: number,
    dataInicial?: Date,
    dataFinal?: Date
  ): Promise<{ agendamentos: Array<RelatorioPagamentoEscritorioResponse>, total: number }> {
    try {
      ...
      return await this.http
        .get<{ agendamentos: Array<RelatorioPagamentoEscritorioResponse>, total: number }>(url)
        .toPromise();
    } catch (error) {
      return error;
    }
