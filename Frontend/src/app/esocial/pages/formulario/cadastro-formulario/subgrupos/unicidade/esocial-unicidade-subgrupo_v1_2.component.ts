...
import { UnicidadeModal_v1_2_Component } from '../../subgrupos-modals/unicidade/unicidade-modal_v1_2.component';

@Component({
  selector: 'app-esocial-unicidade-subgrupo-v1-2',
  templateUrl: './esocial-unicidade-subgrupo_v1_2.component.html',
  styleUrls: ['./esocial-unicidade-subgrupo_v1_2.component.css']
})
export class EsocialUnicidadeSubgrupo_v1_2_Component implements AfterViewInit, OnInit {
  breadcrumb: string;
  constructor(
  ...
  async incluir(): Promise<void> {
    ...
    const teveAlteracao: boolean =
      await UnicidadeModal_v1_2_Component.exibirModalIncluir(this.formularioId, this.contratoId);
  ...
  async alterar(item: Unicidade): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean =
      await UnicidadeModal_v1_2_Component.exibirModalAlterar(this.formularioId, this.contratoId, item, this.temPermissaoEsocialBlocoABCDEFHI );
