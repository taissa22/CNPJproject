...
import { ContribuicaoModal_v1_2_Component } from '../../subgrupos-modals/contribuicao/contribuicao-modal_v1_2.component';

@Component({
  selector: 'app-esocial-contribuicao-subgrupo-v1-2',
  templateUrl: './esocial-contribuicao-subgrupo_v1_2.component.html',
  styleUrls: ['./esocial-contribuicao-subgrupo_v1_2.component.scss']
})
export class EsocialContribuicaoSubgrupo_v1_2_Component implements AfterViewInit {
 ...
 async buscarTabela() {
  ...
  async incluir(): Promise<void> {
    ...
    const teveAlteracao: boolean = await ContribuicaoModal_v1_2_Component.exibeModalIncluir(this.codF2501, this.codCalTrib);
  ...
  async alterar(codInfoContrib: number): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean = await ContribuicaoModal_v1_2_Component.exibeModalAlterar(this.codF2501, this.codCalTrib, codInfoContrib, this.temPermissaoBlocoCeDeE);
    if (teveAlteracao) {
      this.buscarTabela();
    }
  }
