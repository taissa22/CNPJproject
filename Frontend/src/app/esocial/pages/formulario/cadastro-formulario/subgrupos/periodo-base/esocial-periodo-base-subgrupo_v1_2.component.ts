...
import { PeriodoBaseModal_v1_2_Component } from '../../subgrupos-modals/periodo-base-modal/periodo-base-modal_v1_2.component';

@Component({
  selector: 'app-esocial-periodo-base-subgrupo-v1-2',
  templateUrl: './esocial-periodo-base-subgrupo_v1_2.component.html',
  styleUrls: ['./esocial-periodo-base-subgrupo_v1_2.component.scss']
})
export class EsocialPeriodoBaseSubgrupo_v1_2_Component implements AfterViewInit {
  ...
  async carregarBlocoPeriodo(): Promise<void> {
    ...
    const teveAlteracao: boolean = await CarregarBloco_v1_2_Component.exibeModal(this.formularioId, "periodo-base");
  ...
  async carregarBlocoContribuicao(): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean = await CarregarBloco_v1_2_Component.exibeModal(this.formularioId, "contribuicao");
  ...
  async incluir(): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean = await PeriodoBaseModal_v1_2_Component.exibeModalIncluir(this.formularioId);
  ...
  async alterar(codF2501: number, codCalTrib: number): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean = await PeriodoBaseModal_v1_2_Component.exibeModalAlterar(codF2501, codCalTrib, this.temPermissaoBlocoCeDeE);
    if (teveAlteracao) {
      await this.buscarTabela();
      this.carregado.emit(true);
    }
