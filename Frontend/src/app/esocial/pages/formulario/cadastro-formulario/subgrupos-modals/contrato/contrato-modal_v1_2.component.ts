...
@Component({
  selector: 'app-contrato-modal-v1-2',
  templateUrl: './contrato-modal_v1_2.component.html',
  styleUrls: ['./contrato-modal_v1_2.component.scss']
})
export class ContratoModal_v1_2_Component implements AfterViewInit {
  ...
  tooltipMatricula: string = 'Preencher com a matrícula atribuída ao trabalhador pela empresa. Se o tipo de contrato for 9- Trabalhador cujos contratos foram unificados, preencher com a matrícula que incorporará as demais. Para processos de terceiros, caso essa informação seja desconhecida, clique no link ao lado do campo para que o sistema gere uma matrícula fictícia.';
  ...
  possuiInformacaoEventoContratoFormControl: FormControl = new FormControl(
    null,
    Validators.required
  );
  ...
  formGroup: FormGroup = new FormGroup({
    ...
    infocontrIndcontr: this.possuiInformacaoEventoContratoFormControl,
  ...
  ngAfterViewInit(): void {
    ...
    this.desabilitapossuiInformacaoEvento();
  }
  ...
  iniciarForm() {
    if (this.contrato) {
      ...
      this.possuiInformacaoEventoContratoFormControl.setValue(
      ...
      this.matriculaInicial = this.contrato.infocontrMatricula;
  ...
  async salvar() {
    ...
    if (this.matriculaFormControl.value && (this.matriculaInicial != this.matriculaFormControl.value)) {
      const codInterno = localStorage.getItem('codInterno');
      const retorno = await this.service.consultaMatricula(this.matriculaFormControl.value, codInterno);
      if (retorno['mensagem'] != null) {
        const resposta = await this.dialogService.confirmCustom(
          `${retorno['mensagem']}`,
          `A matricula informada já se encontra em uso no <br> Processo nº: <b>${retorno['processo']}</b> (Código Interno: <b>${retorno['codigoInterno']}</b>)<br> 
           com a Parte: <b>${retorno['parteNome']}</b> (CPF: <b>${retorno['parteCpf']}</b>). <br> Deseja realmente continuar?`
          , DialogServiceCssClass.cssDialogo43, true
        );
        if (!resposta) {
          return;
        }
    ...
    try {
      ...
      if (this.tipoContratoFormControl.value != null &&
        (this.tipoContratoFormControl.value === 1 ||
          this.tipoContratoFormControl.value === 2 ||
          this.tipoContratoFormControl.value === 3 ||
          this.tipoContratoFormControl.value === 4)) {
            obj.infocontrIndcontr = 'S';      
      }
      ...
      await this.dialogService.err(
        `Desculpe, não foi possivel realizar a ${Operacao.toLocaleLowerCase()}`,
        mensagem
      );
    }
  }
  ...
  ajustaDataTermindoTSVE(evento: any) {
    if (this.possuiInformacaoEventoContratoFormControl.value == 'S') {
      this.dataTerminoContratoFormControl.setValue(null);
      this.dataTerminoContratoFormControl.disable();
    }else{
      if (
  ...
  ajustaValidators() {
    if ((this.possuiInformacaoEventoContratoFormControl.value != null && this.possuiInformacaoEventoContratoFormControl.value == "N")
        || (this.matriculaFormControl.value == null || this.matriculaFormControl.value == "")) {
    ...
    if (
      (this.tipoContratoFormControl.value != null &&
      this.tipoContratoFormControl.value == 6) ||
      (this.possuiInformacaoEventoContratoFormControl.value != null &&
        this.possuiInformacaoEventoContratoFormControl.value == 'N'))
    {
    ...
    if (
      ...
      this.tipoContratoFormControl.value != 6 &&
      this.possuiInformacaoEventoContratoFormControl.value != null &&
      this.possuiInformacaoEventoContratoFormControl.value == 'S'
    ) {
    ...
    if (this.possuiInformacaoEventoContratoFormControl.value == 'S') {
      ...
      } else if (this.possuiInformacaoEventoContratoFormControl.value != null
        && this.possuiInformacaoEventoContratoFormControl.value == 'N'
        && (this.codigoCategoriaFormControl.value != null
          && this.codigoCategoriaFormControl.value == 721
          || this.codigoCategoriaFormControl.value == 722
    ...
    if (this.possuiInformacaoEventoContratoFormControl.value != null &&
      this.possuiInformacaoEventoContratoFormControl.value == 'N') {
  ...
  disableAll(): void {
    ...
    this.possuiInformacaoEventoContratoFormControl.disable();
    this.dataAdmissaoOriginalFormControl.disable();
    this.reintegracaoEmpregadoFormControl.disable();
  ...
  desabilitapossuiInformacaoEvento(){
    if (this.tipoContratoFormControl.value != null &&
      (this.tipoContratoFormControl.value === 1 ||
        this.tipoContratoFormControl.value === 2 ||
        this.tipoContratoFormControl.value === 3 ||
        this.tipoContratoFormControl.value === 4)) {
          this.possuiInformacaoEventoContratoFormControl.setValue('S');
          this.possuiInformacaoEventoContratoFormControl.disable();      
    }else{
      this.possuiInformacaoEventoContratoFormControl.enable();
    }
