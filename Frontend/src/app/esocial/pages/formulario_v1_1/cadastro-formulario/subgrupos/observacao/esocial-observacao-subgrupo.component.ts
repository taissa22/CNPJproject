...
import { Observacao } from '@esocial/models/subgrupos/observacao';
import { ObservacaoService } from '@esocial/services/formulario_v1_1/subgrupos/observacao.service';
import { SisjurPaginator } from '@libs/sisjur/sisjur-paginator/sisjur-paginator.component';
import { SisjurTable } from '@libs/sisjur/sisjur-table/sisjur-table.component';
import { DialogService } from '@shared/services/dialog.service';
import { ObservacaoModalComponent } from '../../subgrupos-modals/observacao/observacao-modal.component';

@Component({
  selector: 'app-esocial-observacao-subgrupo',
  templateUrl: './esocial-observacao-subgrupo.component.html',
  styleUrls: ['./esocial-observacao-subgrupo.component.css']
})
export class EsocialObservacaoSubgrupoComponent implements AfterViewInit {
  breadcrumb: string;
  constructor(
  ...
  async incluir(): Promise<void> {
    ...
    const teveAlteracao: boolean =
      await ObservacaoModalComponent.exibeModalIncluir(this.formularioId, this.contratoId);
  ...
  async alterar(item: Observacao): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean =
      await ObservacaoModalComponent.exibeModalAlterar(item, this.contratoId, this.formularioId, this.temPermissaoEsocialBlocoABCDEFHI);
