...
import { ContratoModalComponent } from '../../subgrupos-modals/contrato/contrato-modal.component';

@Component({
  selector: 'app-esocial-contrato-subgrupo',
  templateUrl: './esocial-contrato-subgrupo.component.html',
  styleUrls: ['./esocial-contrato-subgrupo.component.css']
})
export class EsocialContratoSubgrupoComponent implements AfterViewInit {
  breadcrumb: string;
  constructor(
  ...
  async incluir(): Promise<void> {
    ...
    const teveAlteracao: boolean = await ContratoModalComponent.exibeModalIncluir(this.formularioId, this.contratoTerceiro);
  ...
  async alterar(item: Contrato): Promise<void> {
    this.aoAbrirPopup.emit();
    const teveAlteracao: boolean = await ContratoModalComponent.exibeModalAlterar(
      item
    );
  ...
  async consultar(item: Contrato): Promise<void> {
    await ContratoModalComponent.exibeModalConsultar(item);
  }
  ...
  async obterDados() {
    try {
      const [
        ...
        this.serviceList.obterRepercussaoProcessoAsync(),
      ...
      this.tipoRegimePrevidenciarioList = tipoRegimePrevidenciarioList;
      this.tipoContratoTempoParcialList = tipoContratoTempoParcialList;
      ...
      this.repercussaoProcessoList = repercussaoProcessoList;
      this.tipoRegimeTrabalhistaList = tipoRegimeTrabalhistaList;
