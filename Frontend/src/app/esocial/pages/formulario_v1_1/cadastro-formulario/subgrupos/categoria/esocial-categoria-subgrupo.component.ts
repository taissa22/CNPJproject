...
import { CategoriaModalComponent } from '../../subgrupos-modals/categoria/categoria-modal.component';

@Component({
  selector: 'app-esocial-categoria-subgrupo',
  templateUrl: './esocial-categoria-subgrupo.component.html',
  styleUrls: ['./esocial-categoria-subgrupo.component.css']
})
export class EsocialCategoriaSubgrupoComponent implements AfterViewInit {
  constructor(
  ...
  async incluir(): Promise<void> {
    ...
    const teveAlteracao: boolean =
      await CategoriaModalComponent.exibeModalIncluir(this.formularioId, this.contratoId);
  ...
  async alterar(item: MudancaCategoria): Promise<void> {
    this.emitAoAbrirPopup();
    const teveAlteracao: boolean =
      await CategoriaModalComponent.exibeModalAlterar(this.formularioId,this.contratoId,item, this.temPermissaoEsocialBlocoABCDEFHI);
