...
export class ESocialCadastroFormulario2500Service {
  ...
  private readonly href: string = `${environment.api_v2_url}/api/esocial/formulario/ESocialF2500`;
  ...
  async retornaStatusRascunhoAsync(idF2500: number): Promise<any> {
    try {
      let url: string = `${this.href}/cadastro/alteracao/f2500/retorna-rascunho/${idF2500}`;
      return await this.http.patch(url, null).toPromise()
    } catch (error) {
      throw error;
    }
  ...
  async alterarNumeroReciboAsync(idF2500: number, nrRecibo?: string): Promise<any> {
    try {
      ...
      let url: string = `${this.href}/cadastro/alteracao/f2500/atualiza-recibo/${idF2500}`;
  ...
  async validaQuantidadeMesesContador(codigoFormulario: number): Promise<string> {
    try {
      let url: string = `${this.href}/validacao/info-contrato-subgrupos/contador/valida-quantidade-meses/${codigoFormulario}`;
