...
export class ImpostoService {
  ...
  async obterImpostoTribtaveis(codF2501: number, codIrrf: number): Promise<Imposto> {
    let url = this.urlConsulta + `/info-cr-irrf/tributaveis/${codF2501}/${codIrrf}`
    return await this.http.get<Imposto>(url).toPromise()
  }
  ...
  async atualizarTributaveis(codF2501: number, codIrrf: number, obj: any): Promise<any> {
    let url = this.urlAlteracao + `/info-cr-irrf/tributaveis/${codF2501}/${codIrrf}`
    try {
      return await this.http.put<any>(url, obj).toPromise();
    } catch (error) {
      throw error;
