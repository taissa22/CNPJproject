...
@Component({
  selector: 'app-manter-tributario-judicial',
  templateUrl: './manter-tributario-judicial.component.html'
})
export class ManterTributarioJudicialComponent implements OnInit {
  ...
  private static exibeModal(): NgbModalRef {
    ...
    const modalRef = modalService.open(ManterTributarioJudicialComponent, { centered: true, backdrop: 'static' });
    return modalRef;
  }
  ...
  private async criar(): Promise<void> {
    try {
      await this.service.criar('tributario-judicial', {
  ...
  private async atualizar(): Promise<void> {
    try {
      await this.service.atualizar('tributario-judicial', {
        id: this.entidade.id,
        descricao: this.descricaoFormControl.value
      });
