...
@Component({
  selector: 'app-fechamento-pex-media',
  templateUrl: './fechamento-pex-media.component.html',
  styleUrls: ['./fechamento-pex-media.component.scss']
})
export class FechamentoPexMediaComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.mensalFechamentoMensal = true;
  ...
  fechamentoMultiplicadorPadraoOutlier: string
  aplicarHaircutGarantia: boolean;
  //#endregion
  ...
  execucaoImediataAgendAutRelMov: boolean = false;
  //#endregion
  ...
  dataEspecificaAgendAutRelMov: boolean = false;
  //#endregion
  ...
  diariaAgendAutRelMov: boolean = false;
  ...
  semanal: boolean;
  semanalDiaPrevia = "";
  semanalAgendAutRelMov: boolean = false;
  //#endregion
  ...
  mensalDiaPrevia = ""
  mensalAgendAutRelMov: boolean = false;
  //#endregion
  ...
  blur(item, name) {
    switch (name) {
      case 'PercHaircut':
        ...
        break;
      case 'MultPadrao':
        item == null ? this.fechamentoMultiplicadorPadraoOutlier = '0' : this.fechamentoMultiplicadorPadraoOutlier;
        break
    }
  }
  ...
  obterHaircutPadrao() {
    this.service.obterHaircutPadrao(51).subscribe(result => {
      this.fechamentoPercHaircut = result.toString().replace('.', ',')
    });
  }

  obterValorPex() {
    this.service.obterValorPex().subscribe(result => {
      this.fechamentoMultiplicadorPadraoOutlier = result.toString().replace('.', ',')
    });
  }
  ...
  salvarFechamento() {
    ...
    fechamentoSubmit.codTipoFechamento = 51;
    ...
    fechamentoSubmit.multDesvioPadrao = this.fechamentoMultiplicadorPadraoOutlier.replace(',', '.');
    fechamentoSubmit.indAplicarHaircutProcGar = this.aplicarHaircutGarantia ? 'S' : 'N';
  ...
  opcaoExecucao(item: string, editar?) {
    ...
    switch (item) {
      ...
      case 'mensalAgendAutRelMov':
        ...
        break        
      case 'aplicarHaircutGarantia':
        check.checked ? this.aplicarHaircutGarantia = true : this.aplicarHaircutGarantia = false;
        break;
    }
  }
  ...
  novovalor(editar?, item?) {
    ...
    this.fechamentoMultiplicadorPadraoOutlier = item.fechamentoMultiplicadorPadraoOutlier;
    this.aplicarHaircutGarantia = item.indAplicarHaircutProcGar
    item.indAplicarHaircutProcGar == "S" ? setTimeout(() => { this.opcaoExecucao('aplicarHaircutGarantia', editar) }, 100) : '';
  }
  ...
  edicaoDados(edicao: boolean) {
   ...
   //  (document.getElementById('execucaoImediataAgendAutRelMov') as HTMLInputElement).checked = false
