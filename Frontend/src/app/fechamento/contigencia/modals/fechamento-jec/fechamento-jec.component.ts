...
@Component({
  selector: 'app-fechamento-jec',
  templateUrl: './fechamento-jec.component.html',
  styleUrls: ['./fechamento-jec.component.scss']
})
export class FechamentoJecComponent implements OnInit, AfterViewInit {
  ...
  fechamentoValorCorteOutlier: number
  fechamentoValorCorteOutlierPadrao: number
  fechamentoObservacoes: string;
  fechamentoObservacoesPadrao: string
  fechamentoObservacoesValidado: boolean = true;
  ...
  blur(item, name) {
    switch (name) {
      case 'PercHaircut':
        if (item == '') this.fechamentoPercHaircut = '0';
        break;
      case 'CorteOutlier':
        if (item == null) this.fechamentoValorCorteOutlier = 0;
        this.validarObservacoes()
        break
    }
  }
  ...
  obterHaircutPadrao() {
    this.service.obterHaircutPadrao(49).subscribe(result => {
  ...
  obterValorOutlier() {
    ...
    this.service.obterValorOutlier().subscribe(result => {

      a.fechamentoValorCorteOutlier = result
      a.fechamentoValorCorteOutlierPadrao = result
      a.fechamentoObservacoesValidado = true;
    });
  }

  validarObservacoes() {
    if (this.fechamentoValorCorteOutlier != this.fechamentoValorCorteOutlierPadrao && this.fechamentoObservacoes == this.fechamentoObservacoesPadrao) {
      return this.fechamentoObservacoesValidado = false
    }
  }
  ...
  salvarFechamento() {
    ...
    fechamentoSubmit.codTipoFechamento = 49;
    ...
    fechamentoSubmit.valCorteOutliers = this.fechamentoValorCorteOutlier;
    fechamentoSubmit.obsValCorteOutliers = this.fechamentoObservacoes == null ? '' : this.fechamentoObservacoes;

    if (this.validarObservacoes() == false) {
      this.messageService.MsgBox2('O valor de corte de outliers foi alterado. Por favor, preencha o campo observação para justificar a alteração.', 'Não foi possível fazer o agendamento', 'warning', 'Ok'); return
    };
  ...
  edicaoDados(edicao: boolean) {
    if (edicao) {
      ...
      this.fechamentoValorCorteOutlier = this.fechamentoEditado.valCorteOutliers
      this.fechamentoValorCorteOutlierPadrao = this.fechamentoEditado.valCorteOutliers
      this.fechamentoObservacoes = this.fechamentoEditado.obsValCorteOutliers
      this.fechamentoObservacoesPadrao = this.fechamentoEditado.obsValCorteOutliers
