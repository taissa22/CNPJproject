...
@Component({
  selector: 'app-fechamento-cc',
  templateUrl: './fechamento-cc.component.html',
  styleUrls: ['./fechamento-cc.component.scss']
})
export class FechamentoCcComponent implements OnInit, AfterViewInit {
  ...
  salvarFechamento() {
    ...
    fechamentoSubmit.codTipoFechamento = 1;
    ...
    if (this.execucaoImediata) {
      ...
      // empresaSubmit.execucaoImediataParcial = this.execucaoImediataDataPrevia.toLocaleDateString("pt-BR").split('/').reverse().join('-') == new Date().toLocaleDateString("pt-BR").split('/').reverse().join('-');
    }
    ...
    if (this.dataEspecifica) {
      ...
      // empresaSubmit.dataEspecificaParcial = this.dataEspecificaDataPrevia.toLocaleDateString("pt-BR").split('/').reverse().join('-') == this.dataEspecificaExecutarData.toLocaleDateString("pt-BR").split('/').reverse().join('-');
    }
    ...
    if (this.mensal) {
      ...
      fechamentoSubmit.diaDoMes = this.mensalUltimoDiaMes == true ? ((new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0)).getDate()) : this.mensalDiaPrevia;
      if (fechamentoSubmit.diaDoMes == null) { this.messageService.MsgBox2('Data da prévia não informada.', 'Não foi possível fazer o agendamento', 'warning', 'Ok'); return; }
    }
  ...
  opcaoExecucao(item: string, editar?) {
    ...
    switch (item) {
      ...
      case 'mensalUltimoDiaMes':
        ...
        this.mensalUltimoDiaMes == true ? this.mensalDiaPrevia = null : this.mensalDiaPrevia;
        break;
