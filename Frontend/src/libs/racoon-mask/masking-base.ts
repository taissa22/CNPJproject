export class MaskingBase {
  ...
  public checkValue(onFocus = false) {
    ...
    if (
      !this.focus &&
      this.oldValue.length < this._input.value.length &&
      !this.isInputKeyAcceptable()
    ) {
    ...
    if (
      this._overwriteOnInsert &&
      this._input.selectionStart < this._input.value.length &&
      !this.focus
    ) {
      let selectionStart = this._input.selectionStart;
      if (
        !MaskingBase.isAlpha(this.value.charAt(selectionStart)) &&
        !MaskingBase.isNumeric(this.value.charAt(selectionStart))
      ) {
        selectionStart++;
      }
      this.value =
  ...
  private isInputKeyAcceptable(): boolean {
    if (
      ...
      !MaskingBase.isNumeric(
        this._input.value.charAt(this._input.selectionStart - 1)
      )
    ) {
      return false;
    }
    ...
    if (
      this.getConstChars().includes(
        this._mask.charAt(this._input.selectionStart - 1)
      )
    ) {
      if (
  ...
  public maskValue() {
    ...
    for (let i = 0; i < this._mask.length && this.value.length !== i; i++) {
      ...
      if (
        ...
        maskChar !== valueChar &&
        maskedValue.charAt(i + dif) !== maskChar
      ) {
  ...
  private getUpdatedCaretPos(maskedValue: string) {
    ...
    if (
      this._mask.charAt(caretPos - 1) !== '9' &&
      this._mask.charAt(caretPos - 1) !== 'A' &&
      caretPos === startCaretPos
    ) {
      caretPos++;
    }
    return caretPos;
